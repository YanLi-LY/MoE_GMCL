...
from minigpt4.models.moe import Moe
...
class MiniGPT4(Blip2Base):
    ...
    def __init__(
        ...
        self.visual_encoder, self.ln_vision = self.init_vision_encoder(
            vit_model, img_size, drop_path_rate, use_grad_checkpoint, vit_precision, cfg
        )
        if freeze_vit:
        ...
        self.llama_proj = Moe(img_f_dim, self.llama_model.config.hidden_size, r=cfg.lora_rank, lora_nums=cfg.lora_nums, task_nums=cfg.task_num)
    ...
    def encode_img(self, image, task_id=None, train=False):
        device = image.device


        with self.maybe_autocast():
            image_embeds, route_weight = self.visual_encoder(image, train=train)
            image_embeds = image_embeds.to(device)
            if train:
                route_weight = F.one_hot(task_id, self.visual_encoder.task_num)
            image_embeds = self.ln_vision(image_embeds).to(device) # B, 257, 1408
            if self.has_qformer:
                image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(device) # B, 257
                ...
                inputs_llama, kd_loss = self.llama_proj(query_output.last_hidden_state, route_weight, train)
            else:
        ...
        if train:
            return inputs_llama, atts_llama, route_weight, kd_loss
        else:
            return inputs_llama, atts_llama, route_weight

    def freeze_before_ln(self, task_id):
        for i in range(task_id):
            for n, p in self.llama_proj_list[i].named_parameters():
    ...
    def forward(self, samples, train=False):
        if self.linear:
            image = samples["image"]
            img_embeds, atts_img = self.encode_img(image)
        ...
        image = samples["image"]
        task_id = samples["task_id"].to(torch.int64)
        img_embeds, atts_img, route_weight, kd_loss = self.encode_img(image, task_id=task_id, train=train)
    ...
    @classmethod
    def from_config(cls, cfg, num_classes=0, train=False, class_per_task=20, task_num=1, pool_size=0):
        ...
        model = cls(
            ...
            lora_r=lora_r,
            lora_alpha=lora_alpha,
            linear=linear, cfg=cfg
        )

        tmp = {}

        for i in range(num_classes):
            task_id = int(i/class_per_task)
            tmp[i] = task_id
        model.visual_encoder.target_task_map.update(tmp)
        model.visual_encoder.init_means_covars(pool_size, cfg.shrink)

        if not train:
            model.visual_encoder.total_classes += num_classes
            model.visual_encoder.task_num += task_num 

        ckpt_path = cfg.get("ckpt", "")  
        ckpt = torch.load('./vicuna-7b/prerained_minigpt4_7b.pth', map_location="cpu")

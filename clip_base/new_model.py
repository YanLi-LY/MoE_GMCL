...
class ClassIncremental(nn.Module):
    def __init__(self, cfg, device, classnames, jit=False):
        ...
        self.model, self.transforms = get_clip(device=device, model_name=cfg.model_name)
    ...
    def forward(self, image, is_train=True, task_id=0, targets=[]):
                ...
                for idx_im in range(len(image_features)):
                    ...
                    while i >= 0:
                        ...
                        if cur_tar != self.increment and i != 0:
                            ...
                            if indices[0] in cur_range or indices[1] in cur_range: 

...
if args.task_id==0:
    class_per_task = cfg.origin_config.initial_increment * cfg.origin_config.n_clusters
    num_classes = cfg.origin_config.initial_increment
else:
    class_per_task = cfg.origin_config.increment * cfg.origin_config.n_clusters
    num_classes = cfg.origin_config.initial_increment + args.task_id * class_per_task
model_config = cfg.model_cfg
model_config.device_8bit = args.gpu_id
model_config.ckpt = args.ckpt_path
model_cls = registry.get_model_class(model_config.arch)
model = model_cls.from_config(model_config, num_classes, class_per_task=class_per_task, task_num=args.task_id+1, pool_size=cfg.origin_config.class_num).to('cuda:{}'.format(args.gpu_id))
# model = model_cls.from_config(model_config, num_classes, class_per_task=class_per_task, task_num=args.task_id+1, pool_size=cfg.origin_config.task_num).to('cuda:{}'.format(args.gpu_id))
